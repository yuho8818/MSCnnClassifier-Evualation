...
def DenseNet(blocks,
    ...
    x = Conv2D(64, 7, strides=2, use_bias=False, name=str(name_prefix+'conv1/conv'))(x)
    x = BatchNormalization(axis=bn_axis, epsilon=1.001e-5,
                           name=str(name_prefix+'conv1/bn'))(x)
    x = Activation('relu', name=str(name_prefix+'conv1/relu'))(x)
    x = ZeroPadding2D(padding=((1, 1), (1, 1)))(x)
    x = MaxPooling2D(3, strides=2, name=str(name_prefix+'pool1'))(x)

    x = dense_block(x, blocks[0], name=str(name_prefix+'conv2'))
    x = transition_block(x, 0.5, name=str(name_prefix+'pool2'))
    x = dense_block(x, blocks[1], name=str(name_prefix+'conv3'))
    x = transition_block(x, 0.5, name=str(name_prefix+'pool3'))
    x = dense_block(x, blocks[2], name=str(name_prefix+'conv4'))
    x = transition_block(x, 0.5, name=str(name_prefix+'pool4'))
    x = dense_block(x, blocks[3], name=str(name_prefix+'conv5'))

    x = BatchNormalization(axis=bn_axis, epsilon=1.001e-5,
                           name=str(name_prefix+'bn'))(x)

    if include_top:
        x = GlobalAveragePooling2D(name=str(name_prefix+'avg_pool'))(x)
        x = Dense(classes, activation='softmax', name=str(name_prefix+'fc1000'))(x)
    else:
        if pooling == 'avg':
            x = GlobalAveragePooling2D(name=str(name_prefix+'avg_pool'))(x)
        elif pooling == 'max':
            x = GlobalMaxPooling2D(name=str(name_prefix+'max_pool'))(x)
    ...
    if blocks == [6, 12, 24, 16]:
        model = Model(inputs, x, name=str(name_prefix+'densenet121'))
    elif blocks == [6, 12, 32, 32]:
        model = Model(inputs, x, name=str(name_prefix+'densenet169'))
    elif blocks == [6, 12, 48, 32]:
        model = Model(inputs, x, name=str(name_prefix+'densenet201'))
    else:
        model = Model(inputs, x, name=str(name_prefix+'densenet'))
...
def DenseNet121(include_top=True,
    ...
    return DenseNet([6, 12, 24, 16],
                    ...
                    pooling, classes,name_prefix=name_prefix)
